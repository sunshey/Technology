...
/**
 * Created by wanglin  on 2017/6/7 09:34.
 */
...
public class AsyncImageLoader {
    ...
    // 内存缓存默认 10M
    private static final int MEM_CACHE_DEFAULT_SIZE = 10 * 1024 * 1024;
    // 文件缓存默认 100M
    private static final int DISK_CACHE_DEFAULT_SIZE = 100 * 1024 * 1024;
    // 一级内存缓存基于 LruCache
    ...
    public Bitmap loadImage(ImageView imageView, String imageUrl) {
        ...
        if (bitmap != null) {
            LogUtil.i("image exists in memory");
            return bitmap;
        }
        ...
        if (bitmap != null) {
            LogUtil.i("image exists in file");
            // 重新缓存到内存中
