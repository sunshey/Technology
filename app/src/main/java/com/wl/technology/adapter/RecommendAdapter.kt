...
class RecommendAdapter(context: Context?, mList: List<DataItem>?) : BaseAdapter<DataItem>(context, mList) {
    ...
    override fun convert(holder: BaseViewHolder?, position: Int) {
        ...
        DbUtils.queryDataById(dataItem._id).observeOn(AndroidSchedulers.mainThread()).subscribe {
            if (it.isRead) {
        ...
        holder.itemView.setOnClickListener {
            ...
            Observable.just(dataItem._id).subscribeOn(Schedulers.io()).subscribe { DbUtils.updataData(it, true) }
